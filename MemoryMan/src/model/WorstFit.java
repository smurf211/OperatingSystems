 ...
 * 
 * 
 * The Worst Fit class contains all functions necessary to perform the algorithm. Holes are chosen that are the largest possible size to fit the process.
 * 
 * 
 * @see <A href="../src/model/WorstFit.java">Java sourceCode</A>
 * 
 * 
...
public class WorstFit {
	...
	public WorstFit(int OSsize, int totalMemSize) {
	...
	public HBox addProcess(int size, int processNumber) {
		...
		if (checkValue == -1) {
			...
			Process p1 = new Process(size, processNumber);
			if (holeCheck) {
				// System.out.println(totalMemSize);
				//System.out.println(currentBlockHeight);
				double correctBlockHeight = Double.valueOf(currentBlockHeight) * Double.valueOf(totalMemSize);
				correctBlockHeight = correctBlockHeight / 700;
				//System.out.println(correctBlockHeight);
				int endOfMemHole = (int)totalMemSize - (int)correctBlockHeight;
				holeList.add(checkValue,endOfMemHole);
				int largestHole = holeList.get(0);
				largestHoleIndex = 0;
				...
				for(int i =0; i < holeList.size(); i++) {
					...
					if(holeList.get(i) > largestHole) {
						
						largestHole = holeList.get(i);
						largestHoleIndex = i;
						
					}
				}
				
				//System.out.println("LARGEST" + largestHoleIndex + " " + largestHole);
				if(largestHoleIndex == memBlockBag.size()) {
					
					memBlockBag.add(p1);
				}
				else if (p1.getSize() == memBlockBag.get(largestHoleIndex).getSize()) {

					memBlockBag.set(largestHoleIndex, p1);

				}

				else {
					
					sizeOfNewHole = memBlockBag.get(largestHoleIndex).getSize() - p1.getSize();
					memBlockBag.set(largestHoleIndex, p1);

					Hole hole = new Hole(sizeOfNewHole);
					memBlockBag.add(largestHoleIndex + 1, hole);
		...
		double blockHeight = percent * 700;
		if (!holeCheck || largestHoleIndex == memBlockBag.size()-1) {
			this.currentBlockHeight += blockHeight;
		}
	...
	public int getLargestHoleIndex() {
